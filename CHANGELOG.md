...
### Added
- **Кросс-платформа, Phase 2: WhatsApp Cloud API адаптер** (v0.51.0-beta.26) — порт бота на WhatsApp поверх нейтрального messaging-слоя (Phase 0, beta.21). Новый teloxide-free модуль `doracore::messaging::whatsapp` реализует traits `Messenger` (outbound) и `InboundSource` (webhook→neutral) против Meta WhatsApp Cloud (Graph) API — весь существующий платформо-нейтральный download/convert/send core переиспользуется без изменений. Слои: `wire` (чистые билдеры Cloud-API JSON — text / interactive reply-buttons / list / media, без I/O), `format` (HTML/`TextStyle`→WhatsApp-разметка `*bold*`/`_italic_`/```` ```mono``` ```` + клампы длин полей), `mime` (инференс MIME для аплоада), `client` (reqwest I/O: `send_message`, `upload_media`, `download_media`), `inbound` (нормализация `entry[].changes[].value.messages[]` → text / button_reply+list_reply→`Action` / document; статусы-доставки пропускаются), `webhook` (subscription `hub.challenge` + `X-Hub-Signature-256` HMAC-SHA256 в constant-time), `server` (axum-роутер `GET` verify + `POST` verify-signature→normalize→`mpsc` канал — монтируется в `dorabot` через `nest`). Деградация UI по `Capabilities::WHATSAPP`: ≤3 → reply-buttons, ≤10 → list, >10 → нумерованное текст-меню; нет edit-in-place (edit = новое сообщение); нет delete (no-op); media не несёт кнопок → файл + отдельное actions-сообщение с подписью. Конфиг из env (`WHATSAPP_TOKEN`/`PHONE_ID`/`VERIFY_TOKEN`/`APP_SECRET`, опц. `GRAPH_BASE`/`GRAPH_VERSION`), секреты в `SecretString`. 60 unit-тестов + 6 интеграционных (реальный reqwest-путь против локального axum-мока Graph API) + 4 webhook-роутер-теста. doracore `0.27.0`→`0.28.0`. План: `~/.claude/plans/rosy-yawning-lemon.md`.
