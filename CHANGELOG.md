...
### Added
- **Playlist Sync — шаг 1: унифицированное ядро + Spotify-провайдер** (v0.51.0-beta.15) — фундамент под ⭐ Подписки (sub-project B). Новый `doracore::playlist`: нейтральные типы (`Platform`, `PlaylistRef`, `PlaylistTrack` с ISRC, `PlaylistSnapshot`), trait `PlaylistProvider` + `PlaylistRegistry` (URL-роутинг, зеркалит `DownloadSource`/`SourceRegistry`). `spotify.rs` — провайдер на Client Credentials (app-token без OAuth юзера, читает публичные плейлисты): `parse_playlist_id` (URL→id, поддержка `?si=`/locale-сегментов), `app_token`, `fetch` (имя + первая страница ≤100 треков), `map_tracks_json` (Spotify JSON → треки с ISRC/длительностью/артистами). 5 unit-тестов (parse URL, маппинг с ISRC + skip null, пусто). Дормантный код — ещё не подключён к UI; следующие шаги: download-bridge (трек→YouTube через odesli/ytsearch→pipeline), БД-подписки + планировщик + проактивный пинг, вкладка ⭐ Подписки. Конфиг `SPOTIFY_CLIENT_ID/SECRET` уже есть.
