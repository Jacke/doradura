...
commands =
    ...
    .cut_limit_iphone_ringtone = for iPhone ringtones
    .cut_limit_android_ringtone = for Android ringtones
    .cut_limit_video_note = for video notes
    .cut_limit_gif = for GIFs
    .cut_truncated = ⚠️ Requested duration {$total} seconds exceeds the Telegram limit {$kind} ({$limit} sec).\n\n✂️ The first {$actual} seconds will be used.
...
silent-toggle-on = 🔇 Silent downloads: ON
silent-toggle-off = 🔔 Silent downloads: OFF
silent-alert-on = 🔇 Silent mode on. Downloads run quietly — I'll recap them next time you're here.
silent-alert-off = 🔔 Silent mode off. Downloads show progress again.
silent-motd-failed-suffix =  — failed
silent-motd-untitled = untitled
silent-motd-header-done = 📬 *While you were away — { $count } silent download\(s\) ready*
silent-motd-header-failed = 📬 *While you were away — { $count } download\(s\) failed*
silent-motd-header-mixed = 📬 *While you were away — { $done } ready, { $failed } failed*
stories-preparing = 📱 Preparing Instagram Stories…
stories-download-failed = ❌ Couldn't fetch the source video. It may no longer be available.
stories-timeout = ❌ Processing took too long. Try a shorter clip.
stories-cut-failed = ❌ Couldn't split the video into Stories.
stories-no-segments = ❌ Couldn't create any Story segments.
stories-send-failed = ❌ Couldn't send the Story segments.
stories-done = ✅ Done — { $count } stories in 9:16.
stories-capped = ⚠️ Video longer than { $minutes } min — trimmed from the start.
...
explore_tab_foryou = ✨ For You
explore_foryou_loading = 🔮 Picking recommendations for you…
explore_foryou_empty = Download a few tracks first — then I'll suggest similar ones here.
explore_similar_button = 🎧 Similar
explore_similar_title = 🎧 More like this
explore_soon = 🔜 Coming soon
explore_trending_empty = Nothing trending yet — be the first to download something.
explore_hits = { $count } downloads
