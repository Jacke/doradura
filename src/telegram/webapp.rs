...
async fn handle_update_settings(
    ...
    if let Some(send_audio_as_doc) = req.send_audio_as_document {
        db::set_user_send_audio_as_document(
            &conn,
            user_id,
            if send_audio_as_doc { 1 } else { 0 },
        )
...
async fn handle_get_history(
    ...
    let items: Vec<HistoryItem> = history_iter
...
async fn handle_get_admin_stats(
    ...
    let admin_queue: Vec<AdminQueueItem> = queue_iter
        .filter_map(|item| item.ok())
        .collect();
    ...
    let active_queue: usize = count_stmt
