...
use crate::utils::{escape_filename, sanitize_filename};
use crate::progress::{ProgressMessage, DownloadStatus};
use crate::config;
use crate::error::AppError;
use crate::db::{DbPool, save_download_history, self as db};
use crate::cache;
    ...
    if let Some(ref cookies_file) = *config::YTDL_COOKIES_FILE {
        if !cookies_file.is_empty() {
            ...
            if !cookies_path_buf.exists() {
                ...
                let abs_path = cookies_path_buf.canonicalize()
            ...
            args.push(unsafe { 
                    std::mem::transmute::<&str, &'static str>(leaked_path)
            });
...
fn convert_webp_to_jpeg(webp_bytes: &[u8]) -> Result<Vec<u8>, AppError> {
    ...
    let temp_webp = temp_dir.join(format!("temp_webp_{}.webp", 
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis()));
    let temp_jpeg = temp_dir.join(format!("temp_jpeg_{}.jpg", 
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis()));
...
fn compress_thumbnail_jpeg(jpeg_bytes: &[u8]) -> Option<Vec<u8>> {
    ...
    let temp_dir = std::env::temp_dir();
    let temp_input = temp_dir.join(format!("thumb_in_{}.jpg", 
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis()));
    let temp_output = temp_dir.join(format!("thumb_out_{}.jpg", 
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis()));
    ...
    let output = Command::new("ffmpeg")
        .args([
            "-i", temp_input.to_str().unwrap_or(""),
            "-vf", "scale=320:320:force_original_aspect_ratio=decrease",
            "-q:v", "5", // Среднее качество для уменьшения размера
            "-y",
...
fn generate_thumbnail_from_video(video_path: &str) -> Option<Vec<u8>> {
    ...
    let temp_dir = std::env::temp_dir();
    let temp_thumbnail_path = temp_dir.join(format!("thumb_{}.jpg", 
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_millis()));
    ...
    let output = Command::new("ffmpeg")
        .args([
            ...
            "-vframes", "1",
            "-vf", "scale=320:320:force_original_aspect_ratio=decrease",
            "-q:v", "2", // Высокое качество JPEG (2 = высокое, 31 = низкое)
            "-f", "image2",
...
fn find_actual_downloaded_file(expected_path: &str) -> Result<String, AppError> {
    ...
    let parent_dir = path.parent().ok_or_else(|| {
        AppError::Download(format!("Cannot get parent directory for: {}", expected_path))
    })?;
    ...
    for entry in dir_entries {
        if let Ok(entry) = entry {
            ...
            let matches_pattern = file_name_str.starts_with(file_stem) && file_name_str.ends_with(&format!(".{}", file_extension));
    ...
    if found_files.is_empty() {
        ...
        return Err(AppError::Download(format!(
            "Downloaded file not found at {} or in directory", expected_path
        )));
    }
    
    // Если найдено несколько файлов, берем последний (наиболее вероятно новый)
    let actual_path = found_files.last().unwrap().clone();
    log::info!("Found actual downloaded file: {} (searched for: {})", actual_path, expected_path);
...
async fn send_error_with_sticker_and_message(bot: &Bot, chat_id: ChatId, custom_message: Option<&str>) {
    ...
    if let Err(e) = bot.send_sticker(chat_id, InputFile::file_id(teloxide::types::FileId(random_sticker_id.to_string()))).await {
            ...
            let duration_result = loop {
                tokio::select! {
                    ...
                    Some(progress_info) = progress_rx.recv() => {
                        // Обновляем при значимых изменениях (разница >= 5% или первое обновление)
        ...
        match result {
            ...
            Err(e) => {
                ...
                let custom_message = if error_str.contains("Only images are available") {
                    Some("Это видео недоступно для скачивания 😢\n\n\
                    Возможные причины:\n\
                    ...
                    Some("У меня устарела версия загрузчика 😢\n\n\
                    ...
                    Some("YouTube заблокировал бота 🤖\n\n\
        ...
        match response {
            ...
            Err(e) if attempt < max_attempts => {
                ...
                if let Some(retry_after_secs) = extract_retry_after(&error_str) {
                    log::warn!("Rate limit hit when sending {} to chat {}: Retry after {}s. Waiting...", 
                        file_type, chat_id, retry_after_secs);
...
async fn send_video_with_retry(
                ...
                if response.status().is_success() {
                    match response.bytes().await {
                        Ok(bytes) => {
                            ...
                            log::info!("[THUMBNAIL] Successfully downloaded thumbnail: {} bytes ({} KB)", 
                                bytes_vec.len(), bytes_vec.len() as f64 / 1024.0);
                    ...
                    log::warn!("[THUMBNAIL] Thumbnail request failed with status: {}", response.status());
                    None
                }
            }
            Err(e) => {
    ...
    let temp_thumb_path: Option<std::path::PathBuf> = if let Some(ref thumb_bytes) = thumbnail_bytes {
        ...
        let (final_bytes, file_ext) = if format == ImageFormat::WebP {
            ...
            match convert_webp_to_jpeg(thumb_bytes) {
                Ok(jpeg_bytes) => {
                    log::info!("[THUMBNAIL] Successfully converted WebP to JPEG: {} bytes", jpeg_bytes.len());
                    (jpeg_bytes, "jpg")
                }
                Err(e) => {
        ...
        let final_bytes = if final_bytes.len() > 200 * 1024 {
            log::warn!("[THUMBNAIL] Thumbnail too large ({} KB), trying to compress", 
        ...
        let temp_dir = std::env::temp_dir();
        let temp_path = temp_dir.join(format!("thumb_{}.{}", 
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_millis(),
        ...
        if fs::write(&abs_path, &final_bytes).is_ok() {
            log::info!("[THUMBNAIL] Saved thumbnail to temporary file: {:?} ({} bytes)", abs_path, final_bytes.len());
            Some(abs_path)
        } else {
    ...
    if let Some(thumb_path) = temp_thumb_path {
        ...
        if result.is_ok() {
            ...
            let _ = fs::remove_file(&thumb_path);
            log::info!("[THUMBNAIL] Cleaned up temporary thumbnail file after error: {:?}", thumb_path);
        }
    }
    ...
    tokio::spawn(async move {
        ...
        let result: Result<(), AppError> = async {
                    ...
                    if !size_str.is_empty() && size_str != "NA" {
                        if let Ok(file_size) = size_str.parse::<u64>() {
                            ...
                            if file_size > max_size {
                                ...
                                return Err(AppError::Validation(format!("File too large (estimated): {:.2} MB (max: {:.2} MB). Try lower quality.", size_mb, max_mb)));
                            }
                        }
                    }
                }
                                        ...
                                        if let Some(start) = before_size.rfind(|c: char| c.is_ascii_digit() || c == '.' || c == '~') {
                                            ...
                                            if let Ok(size_mb) = size_str.parse::<f64>() {
                                                ...
                                                if size_bytes > max_size {
                                                    ...
                                                    return Err(AppError::Validation(format!("File too large (from list-formats): {:.2} MB (max: {:.2} MB) for {}p. Try lower quality.", size_mb, max_mb, target_height)));
                                                }
                                            }
                                        }
                                    }
                    ...
                    Some(progress_info) = progress_rx.recv() => {
                        ...
                        if let Some(total_size) = progress_info.total_size {
                            ...
                            if total_size > max_size {
                                ...
                                return Err(AppError::Validation(format!("File too large (detected during download): {:.2} MB (max: {:.2} MB). Try lower quality.", size_mb, max_mb)));
                            }
                        }
                    }
                    // Ждем завершения загрузки
            ...
            if file_size > max_size {
                ...
                return Err(AppError::Validation(format!("File too large: {:.2} MB (max: {:.2} MB). Try lower quality.", size_mb, max_mb)));
            }
...
fn generate_file_name_with_ext(title: &str, artist: &str, extension: &str) -> String {
    ...
    log::debug!("Generating filename: title='{}' (len={}), artist='{}' (len={}), ext='{}'", 
               title, title.len(), artist, artist.len(), extension);
...
pub async fn download_and_send_subtitles(bot: Bot, chat_id: ChatId, url: Url, rate_limiter: Arc<RateLimiter>, _created_timestamp: DateTime<Utc>, subtitle_format: String, db_pool: Option<Arc<DbPool>>) -> ResponseResult<()> {
    ...
    tokio::spawn(async move {
        ...
        let result: Result<(), AppError> = async {
            ...
            tokio::spawn(async move {
                let _ = msg_for_clear.clear_after(&bot_for_clear, 10, title_for_clear.as_ref().to_string(), Some(subtitle_format_clone)).await;
            });
