...
pub async fn create_subscription_invoice(
    ...
    log::info!("🎯 create_subscription_invoice called for chat_id: {}, plan: {}", chat_id.0, plan);
    ...
    let invoice_link_result = bot.create_invoice_link(
        title,
        description,
        payload,
...
pub async fn handle_successful_payment(
    ...
    if let Some(payment) = msg.successful_payment() {
        ...
        if parts.len() == 3 && parts[0] == "subscription" {
            ...
            log::info!("Processing subscription payment for user {}, plan: {}", telegram_id, plan);
            ...
            if let Err(e) = db::update_telegram_charge_id(
                &conn,
                telegram_id,
                Some(&charge_id_str),
            ) {
            ...
            if let Err(e) = activate_subscription(
                Arc::clone(&db_pool),
                telegram_id,
                plan,
                30,
...
pub async fn cancel_subscription(
    ...
    let charge_id = user.telegram_charge_id
