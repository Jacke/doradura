...
pub async fn download_and_send_video(
    ...
    // Run inline (awaited) — queue_processor needs this function to return only
    // AFTER the download+upload completes. The prior fire-and-forget spawn caused
    // queue_processor to release the permit ~50ms in, letting N downloads run in
    // parallel despite max_concurrent=1 and leaving the UI status "hanging".
    ...
    async move {
        ...
        let result: Result<(), AppError> = match timeout(global_dl_timeout, async {
            ...
            // Experimental features graduated to main workflow — always use hqdefault.jpg
            // (~20-40 KB) instead of maxresdefault.jpg (~200-300 KB) to skip the
            // compress step in send.rs (~0.6s saved).
