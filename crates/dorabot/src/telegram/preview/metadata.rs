...
async fn get_preview_metadata_inner(
    ...
    // ── Fast YouTube metadata (Piped → oEmbed → yt-dlp fallback) ──
    // Piped gives ~1-2s with formats, oEmbed ~200ms with title only.
    // Both bypass yt-dlp's 5-15s JS challenge solving.
    if let Some(fast) = doracore::download::fast_metadata::get_fast_youtube_metadata(url.as_str()).await {
        let fast_source = fast.source;
        log::info!(
            "⚡ Fast preview from {:?} for {} ({})",
            fast_source,
            url,
            if fast.video_formats.is_empty() {
                "no formats".to_string()
            } else {
                format!("{} formats", fast.video_formats.len())
            }
        );

        // Check duration limit (same as yt-dlp path)
        if !has_time_range {
            if let Some(dur) = fast.duration_secs {
        ...
        // Convert Piped video formats to our VideoFormatInfo
        let video_formats: Option<Vec<VideoFormatInfo>> = if !fast.video_formats.is_empty() {
            let converted: Vec<VideoFormatInfo> = fast
                .video_formats
                .iter()
                .filter_map(|f| {
                    let quality = f.quality.trim_end_matches('p');
                    if quality.parse::<u32>().is_err() {
                        return None;
                    }
                    Some(VideoFormatInfo {
        ...
        let filesize = if format == Some("mp3") {
            fast.duration_secs.map(|d| (d as u64) * 320_000 / 8)
        } else {
            None
        };

        let metadata = PreviewMetadata {
            title: fast.title.clone(),
            artist: fast.author.unwrap_or_default(),
            thumbnail_url: fast.thumbnail_url,
            duration: fast.duration_secs,
            filesize,
            description: None,
            video_formats,
            timestamps: vec![],
