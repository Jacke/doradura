...
pub(super) async fn admin_login_handler(State(_state): State<WebState>) -> Response {
    ...
    // HTML template lives in a sibling `.html` file — easier to edit, gets
    // proper syntax highlighting, no `{{` / `}}` escaping for CSS braces,
    // and `include_str!` bakes it into the binary at compile time.
    const LOGIN_HTML: &str = include_str!("html/admin_login.html");
    let html = LOGIN_HTML.replace("{BOT_USERNAME}", &bot_username);
